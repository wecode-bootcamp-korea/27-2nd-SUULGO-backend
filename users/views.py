...
class KakaoLoginView(View):
    def get(self, request):
        try:
            ...
            # user = kakao_client.get_user()
            # location = kakao_client.get_location()
            ...
            # user 객체를 활용해서 profile_image_url 대조 후 update
            # User.objects.filter(kakao_id=kakao_id).update(
            #     profile_image_url = profile_image_url
            # )

            payload = {'id': user.id, 'exp': asdfnasdf, 'iat': asdufnsdnf}
...
class ProductView(View):
    ...
    def get(self, request, product_id):
        try:
            ...
            user_survey.drinking_methods.all()
            user_survey.alcohol_categories.all()
            ...
            result = {
                ...
                "text_stack"               : product.stack.name,
                "alcohol_limit"            : {
                    "name"        : product.alcohol_limit.name,
...
class UserListView(View): 
    def get(self, request):
        ...
        users = User.objects.filter(survey__surveyalcoholcategory__alcohol_category_id=alcohol_category_id)\
                            .prefetch_related('survey_set')[offset:offset+limit]
